...
install_dependencies() {
    ...
    if [ ! -f "pyproject.toml" ]; then
        print_error "pyproject.toml not found. Are you in the Airlift project directory?"
        exit 1
    fi
...
build_application() {
    ...
    # Check if airlift.spec exists
    if [ ! -f "airlift.spec" ]; then
        print_error "airlift.spec not found. Cannot build binary."
        exit 1
    fi
    ...
    if [ -f "$TEST_BUILD_DIR/airlift" ]; then
        print_success "Build completed successfully!"
        print_success "Binary created: $TEST_BUILD_DIR/airlift"
        
        # Make binary executable
        chmod +x "$TEST_BUILD_DIR/airlift"
        
        # Show binary info
        ls -la "$TEST_BUILD_DIR/airlift"
        print_status "Binary size: $(du -h "$TEST_BUILD_DIR/airlift" | cut -f1)"
    else
...
show_usage() {
    ...
    echo "  without requiring actual API calls. If .build/ doesn't exist, it will"
...
main() {
    ...
    print_status "To run the application: ./$TEST_BUILD_DIR/airlift"
...
if [ ! -f "pyproject.toml" ]; then
    print_error "Please run this script from the Airlift project root directory"
    exit 1
fi
